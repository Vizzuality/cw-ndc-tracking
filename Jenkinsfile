...
node {
  ...
  try {
    ...
    // Notify Success
    slackSend (color: '#00FF00', channel: '#cw-ndc-tracker-dev', message: "SUCCESSFUL: Job '${env.JOB_NAME} [${env.BUILD_NUMBER}]' (${env.BUILD_URL})")
    emailext (
    ...
    // Notify Error
    slackSend (color: '#FF0000', channel: '#cw-ndc-tracker-dev', message: "FAILED: Job '${env.JOB_NAME} [${env.BUILD_NUMBER}]' (${env.BUILD_URL})")
    emailext (
